...
namespace MageFactory.UI.Shared.Popup {
    public class PopupManager : MonoBehaviour {
        ...
        [Header("Parent (root Canvas)")] [SerializeField]
        private Canvas parentCanvas; // główny Canvas sceny

        [SerializeField] private RectTransform parentRect; // jego RectTransform

        [Header("Prefab & Pool")] [SerializeField]
        private PopupBase popupPrefab; // prefab pojedynczego popupu (bez Canvas!)
        ...
        /// </summary>
        public void Show(Component owner, string textToShow, Color color, Vector2? offset = null, float moveY = 50f,
                         float duration = 1f) {
            Show(owner.transform, textToShow, color, offset, moveY, duration);
        }

        public void Show(Transform ownerTransform, string textToShow, Color color, Vector2? offset = null,
                         float moveY = 200f, float duration = 1f) {
